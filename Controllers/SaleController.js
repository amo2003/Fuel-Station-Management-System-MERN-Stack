...
const recordSale = async (req, res) => {
  ...
  try {
    ...
    // Deduct sold quantity from fuel storage
    const fuel = await FuelStorage.findOne({ type });
    if (!fuel) {
      return res.status(404).json({ message: "Fuel type not found" });
    }

    fuel.quantity = Math.max(fuel.quantity - Number(soldQuantity), 0); // prevent negative quantity
    await fuel.save();
...
const updateSale = async (req, res) => {
  ...
  try {
    // Find old sale record
    const oldSale = await DailySale.findById(id);
    if (!oldSale) {
      return res.status(404).json({ message: "Sale record not found" });
    }

    // Revert fuel quantity for old sale
    ...
    // Deduct fuel quantity for new sale info
...
const deleteSale = async (req, res) => {
  ...
  try {
    ...
    // Add back sold quantity to fuel storage
